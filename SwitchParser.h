...
#ifndef _switchparser_h_
#define _switchparser_h_

#include "Usb.h"
#include "controllerEnums.h"

/** Buttons on the controller */
const uint8_t SWITCH_BUTTONS[] PROGMEM = {
        UP, // UP
        RIGHT, // RIGHT
        DOWN, // DOWN
        LEFT, // LEFT
...
public:
        ...
        uint16_t getX(uint8_t finger = 0, uint8_t xyId = 0) {
                return switchData.xy[xyId].finger[finger].x;
        };
        ...
        uint16_t getY(uint8_t finger = 0, uint8_t xyId = 0) {
                return switchData.xy[xyId].finger[finger].y;
        };
        ...
        bool isTouching(uint8_t finger = 0, uint8_t xyId = 0) {
                return !(switchData.xy[xyId].finger[finger].touching); // The bit is cleared when a finger is touching the touchpad
        };
        ...
        uint8_t getTouchCounter(uint8_t finger = 0, uint8_t xyId = 0) {
                return switchData.xy[xyId].finger[finger].counter;
        };
        ...
        float getAngle(AngleEnum a) {
                if (a == Pitch)
                        return (atan2f(switchData.accY, switchData.accZ) + PI) * RAD_TO_DEG;
                else
                        return (atan2f(switchData.accX, switchData.accZ) + PI) * RAD_TO_DEG;
        };
        ...
        int16_t getSensor(SensorEnum s) {
                switch(s) {
                        case gX:
                                return switchData.gyroX;
                        case gY:
                                return switchData.gyroY;
                        case gZ:
                                return switchData.gyroZ;
                        case aX:
                                return switchData.accX;
                        case aY:
                                return switchData.accY;
                        case aZ:
                                return switchData.accZ;
                        default:
                                return 0;
                }
        };
        ...
        uint8_t getBatteryLevel() {
                return switchData.status.battery;
        };
         ...
         */
        bool getAudioStatus() {
                return switchData.status.audio;
        };
        ...
        void setRumbleOn(uint8_t bigRumble, uint8_t smallRumble) {
                switchOutput.bigRumble = bigRumble;
                switchOutput.smallRumble = smallRumble;
                switchOutput.reportChanged = true;
        };
        ...
        void setLedFlash(uint8_t flashOn, uint8_t flashOff) {
                switchOutput.flashOn = flashOn;
                switchOutput.flashOff = flashOff;
                switchOutput.reportChanged = true;
        };
        /**@}*/
...
protected:
         ...
         */
        virtual void sendOutputReport(SwitchOutput *output) = 0;

private:
        bool checkDpad(ButtonEnum b); // Used to check PS4 DPAD buttons

        SwitchData switchData;
        SwitchButtons oldButtonState, buttonClickState;
